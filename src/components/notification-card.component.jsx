...
const NotificationCard = ({ data, index, notificationState, }) =>{
    ...
    const handleDelete = (reply_id, type, target) => {

        target.setAttribute("disabled", true)

        axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/delete-comment", { _id: reply_id }, {
            headers: {
    ...
    return (
        <div className={"p-6 border-b border-grey border-l-black " + ( !seen ? "border-l-2" : "" ) }>
            <div className="flex gap-5 mb-3">
                ...
                <div className="w-full">
                        ...
                        <span className="font-normal">
                            {
                                ...
                                type == 'new_blog' ? "published a new blog" :
                    ...
                    </h1>
                    {
                        ...
                        : 
                        type !== 'follow' && <Link className="font-medium text-dark-grey hover:underline line-clamp-1" to={`/blog/${blog_id}`}>{`"${title}"`}</Link>
...

            {
                ...
                <p className="ml-14 pl-5 font-gelasio text-xl mt-5">{comment.comment}</p>
            ...
            <div className="ml-14 pl-5 mt-3 text-dark-grey flex gap-8">
                ...
                {
                    type !== 'like' && type !== 'new_blog' && type !== 'follow' &&
                    <>
                        {
                            !reply ? 
                            <button className="underline hover:text-black" onClick={handleReplyClick}>Reply</button> : ""
                        }
                        <button className="underline hover:text-black" onClick={(e) => handleDelete(comment._id, type="comment", e.target)}>Delete</button>
                    </>
                }
...

                {
                    ...
                    <div className="ml-20 p-5 bg-grey mt-5 rounded-md">
                        ...
                        <button className="underline hover:text-black ml-14 mt-2" onClick={(e) => handleDelete(reply._id, type="reply", e.target)}>Delete</button>
